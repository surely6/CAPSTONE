...
$studentId = "1"; // session id

//all tab
$allBookmarksSql = "SELECT bookmarks.*, learning_materials.material_title AS material_title, learning_materials.material_subject AS material_subject, quizzes.quiz_title AS quiz_title, quizzes.quiz_subject AS quiz_subject, instructors.instructor_name,
                    CASE 
                ...
                FROM bookmarks
                LEFT JOIN learning_materials ON bookmarks.material_id = learning_materials.material_id
                LEFT JOIN quizzes ON bookmarks.quiz_id = quizzes.quiz_id
                LEFT JOIN instructors ON instructors.instructor_id = COALESCE(learning_materials.instructor_id, quizzes.instructor_id)
                WHERE student_id = '$studentId'";
$allBookmarks = mysqli_query($connect, $allBookmarksSql);
...
$materialsSql = "SELECT bookmarks.*, learning_materials.material_title, learning_materials.material_subject, instructors.instructor_name, 
            ...
            WHERE student_id = '$studentId' AND bookmarks.quiz_id IS NULL";
$materials = mysqli_query($connect, $materialsSql);
...
$quizzes = mysqli_query($connect, $quizzesSql);
?>
...
<body>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <a class="bookmarkBtn <?php echo $row['is_bookmarked'] ? 'bookmarked' : ''; ?>"
                                        href="#;"
                                        onclick="toggleBookmark(this, '<?php echo $row['material_id'] ?? $row['quiz_id']; ?>', '<?php echo $studentId; ?>')">
    ...
    <script>
        ...
        //bookmark btn
        function toggleBookmark(element, contentId, studentId) {
            ...
            const cardType = card.querySelector('.card-text:nth-child(4)') ?
                card.querySelector('.card-text:nth-child(4)').textContent.includes('Material') ?
                    'material' : 'quiz' :
                (element.closest('#materials') ? 'material' : 'quiz');
        ...
        document.addEventListener('DOMContentLoaded', function () {
            function handleCardClick(event) {
                ...
                for (let i = 0; i < cardTexts.length; i++) {
                    if (cardTexts[i].textContent.includes('Type:')) {
                        cardType = cardTexts[i].textContent.includes('Material') ? 'material' : 'quiz';
                        break;
                    }
                }

                if (cardType === 'material') {
                    if (card.closest('#quizzes')) {
                        cardType = 'quiz';
                    }
                }

                if (cardType === 'material') {
                    window.location.href = `materialDetails.php?material_id=${contentId}`;
                } else {
                    window.location.href = `quizDetails.php?quiz_id=${contentId}`;
                }
            }
