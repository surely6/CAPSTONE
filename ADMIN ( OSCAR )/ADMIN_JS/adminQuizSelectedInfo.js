...
function SetContent(){
    ...
    let content = `<div id="content">
                    <div class="title">
                        <h1>SUBJECT: `+Quiz['subject'].toUpperCase()+`</h1>
                        <h1>FORM: `+Quiz['form']+`</h1>
                    </div>
                    <div class="section">
...
function SetQuestions(){
    ...
    Quiz['question'].forEach((element,index) => {
        content = content + `
        <div id="part">
            <h1>QUESTION 1<div style="float: right;">`+Quiz['type'][index]+`</div></h1>
...
function SetAnswers(){
    ...
    section.forEach((element,index) => {
        ...
        Quiz['answer'][index].forEach((ans,i) => {
            console.log(Quiz['correct'][index][(i+1)])
            if ((Quiz['correct'][index][(i+1)]) == n){
                inputs = inputs + `<div style="background-color: var(--green); color: var(--grey);">`+ans+`</div>`
            } else {
                inputs = inputs + `<div>`+ans+`</div>`
            }
            n++;
        })
        ...
        if (time == "latest"){
            StudentPass.forEach((element) => {
                ...
                <tr>
                    <td>`+no+`</td>
                    <td>`+element['name']+`</td>
                    <td style="color: var(--green);">`+element['score']+`/`+TotalQuestion+`</td>
                    <td>`+element['date']+`</td>
               </tr>
                `;
                no ++;
            ...
            StudentFail.forEach((element) => {
                ...
                <tr>
                    <td>`+no+`</td>
                    <td>`+element['name']+`</td>
                    <td style="color: var(--red);">`+element['score']+`/`+TotalQuestion+`</td>
                    <td>`+element['date']+`</td>
               </tr>
                `;
                no ++;
            ...
            for (let i = StudentPass.length-1 ; i>=0 ; i--){
                ...
                <tr>
                    <td>`+no+`</td>
                    <td>`+StudentPass[i]['name']+`</td>
                    <td style="color: var(--green);">`+StudentPass[i]['score']+`/`+TotalQuestion+`</td>
                    <td>`+StudentPass[i]['date']+`</td>
               </tr>
                `;
                no ++;
            }
            for (let i = StudentFail.length-1 ; i>=0 ; i--){
                ...
                <tr>
                    <td>`+no+`</td>
                    <td>`+StudentFail[i]['name']+`</td>
                    <td style="color: var(--red);">`+StudentFail[i]['score']+`/`+TotalQuestion+`</td>
                    <td>`+StudentFail[i]['date']+`</td>
               </tr>
                `;
                no ++;
            }
...
function SetFeedback(){
    ...
    if (time == "latest"){
        Feedback.forEach((element) => {
            detail = detail + `<div id="info">
                                    <h1>`+element['name']+`</h1>
                                    <span>Created On `+element['date']+`</span>
        ...
        for (let i = Feedback.length-1 ; i>=0 ; i--){
            detail = detail + `<div id="info">
                                    <h1>`+Feedback[i]['name']+`</h1>
                                    <span>Created On `+Feedback[i]['date']+`</span>
